...
public class Topic implements Comparable<Topic> {
	...
	public Topic(String text, int popularity) {
	...
	public Topic(String text, int popularity, String id) {
		this(text, 0f, 0f, 0f, 1.0f, popularity, id);
	}

	public Topic(String text, float locX, float locY, float locZ) {
		this(text, locX, locY, locZ, 1.0f, DEFAULT_POPULARITY, "");
	}

	public Topic(String text, float locX, float locY, float locZ, float scale) {
		this(text, locX, locY, locZ, scale, DEFAULT_POPULARITY, "");
	}

	public Topic(String text, float locX, float locY, float locZ, float scale,
			int popularity, String id) {
		this.text = text;
		this.locX = locX;
		this.locY = locY;
		this.locZ = locZ;
	...
	private int popularity; // this is the importance/popularity of the Topic
	private int textSize;
	private float locX, locY, locZ; // the center of the 3D Topic
	private float loc2DX, loc2DY;
	private float scale;
	private float colorR, colorG, colorB, alpha;
	private static final int DEFAULT_POPULARITY = 1;
	private int paramNo; // parameter that holds the setting for this Topic
}
